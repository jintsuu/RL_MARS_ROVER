...
# 1 SARSA success per episode
def run_sarsa_success(env, n_eps, gamma=1.0):
...
# Q-Learning
def run_q_success(env, n_eps, gamma=1.0):
    Q = np.zeros((env.nS, env.nA))
    succ = np.zeros(n_eps)
    for ep in range(n_eps):
        α, ε = alpha_schedule(ep), epsilon_schedule(ep)
...
# First-Visit MC
def run_mc_success(env, n_eps, gamma=1.0):
    Q = np.zeros((env.nS, env.nA))
...
# Double Q-Learning
def run_double_q_success(env, n_eps, gamma=1.0):
...
if __name__ == "__main__":
    ...
    # run
    m_sarsa = moving_average(run_sarsa_success(env, n_episodes))
    m_q     = moving_average(run_q_success(env,    n_episodes))
    m_mc    = moving_average(run_mc_success(env,   n_episodes))
    m_dq    = moving_average(run_double_q_success(env, n_episodes))

    # plot
    plt.figure(figsize=(10,6))
    plt.plot(eps, m_sarsa, label="SARSA")
    plt.plot(eps, m_q,     label="Q-Learning")
    plt.plot(eps, m_mc,    label="First-Visit MC")
    plt.plot(eps, m_dq,    label="Double Q-Learning")
    plt.title("Success Rate (Right Terminal) — Moving Avg window=50")
    plt.xlabel("Episode")
